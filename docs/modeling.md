...
1. **Slick ⇔ PostgreSQL**
   ...
   - `LocalizedText` se serializa como `String` mediante `asJson.noSpaces`; durante la lectura se parsea con Circe. Cualquier error de parseo produce una excepción descriptiva, evitando datos corruptos.
   - `requested_targets` se convierte a/desde `String` aprovechando los encoders/decoders de Circe sobre `Seq[RequestedTarget]`. El valor se persiste como `jsonb`, habilitando índices GIN y consultas estructuradas si más adelante se requieren reportes basados en los objetivos solicitados.

2. **Circe ⇔ JSON de API**
   - Se implementaron encoders/decoders implícitos (`EventNodeJson`) coherentes con la estructura de llaves solicitada.
   - `organization_name` requiere un tratamiento especial: el campo `value` contiene JSON serializado como `String`. Se parsea con Circe (`parseLocalizedText`) durante la lectura y, al serializar, se vuelve a producir la cadena JSON embebida.
   - Las fechas se manejan como `Instant` en ISO-8601. Cualquier error de `parse` devuelve un `DecodingFailure` con el mensaje específico.

## Migraciones con Slick

Las migraciones se describen en Scala aprovechando la función `.schema` disponible en `TableQuery`. El objeto
[`EventNodeMigrations`](../src/main/scala/com/example/eventnode/db/migration/EventNodeMigrations.scala)
construye la descripción `SchemaDescription` a partir de `Tables.eventNodes` y ejecuta `createIfNotExists` sobre la base
de datos. Esto mantiene la definición de la tabla `event_nodes` sincronizada con el modelo y permite inspeccionar las
sentencias SQL generadas mediante el método `schemaStatements` (útil para revisiones o pipelines de despliegue).

Para ejecutar las migraciones se provee un `runMain` específico en [`MigrationApplier`](../src/migrations/scala/com/example/eventnode/db/migration/MigrationApplier.scala).
Este entrypoint se ejecuta con:

```bash
sbt "migrations/runMain com.example.eventnode.db.migration.MigrationApplier"
```

El comando espera una configuración Typesafe con el bloque `event-node.db` (o la clave indicada en la propiedad de
sistema `eventnode.migrations.dbConfig`) y deja preparado el esquema antes de que la aplicación consuma el servicio Slick.
...
2. **Persistencia**: el servicio llama al `EventNodeRepository`, que inserta/actualiza una única fila en `event_nodes` con la colección `requested_targets` serializada como `jsonb`. El servicio asume que las migraciones ya fueron aplicadas por el `MigrationApplier`.
3. **Lectura**: el repositorio obtiene las filas de `event_nodes` y reconstruye directamente el `EventNode` de dominio, listo para serializarse nuevamente con Circe cuando se necesite exponerlo.
